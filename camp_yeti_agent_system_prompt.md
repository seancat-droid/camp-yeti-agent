...
4. Generate an image prompt (or write the on-image text if it's a text-card style post like the reference images) that matches the visual identity section exactly.
...
## COMMENT/DM REPLIES (if paired with a reply tool)
